...
- 人可演奏版文本谱、标准 MIDI（内嵌谱面快照，供桌面版读取）与罗技 / 雷蛇宏导出
- 独立的云存档批量罗技宏导出页：按选中顺序合并歌曲，侧键连按选歌（见 [使用说明](docs/LOGITECH_BATCH.md)）
- 浅色 / 深色主题

## 桌面版自动演奏器

`desktop/` 是 WPF（.NET 9）桌面程序：只接受本站导出的 MIDI，也能直接浏览公开曲库，用 SendInput 向游戏发送口琴按键完成自动演奏。指法与网页端逐字节一致（黄金样本测试保证），详见 [docs/DESKTOP.md](docs/DESKTOP.md)。

网页导出的 `.mid` 现在附带一份 DFHS1 谱面快照（MIDI 文本 meta 事件），桌面端据此还原键位与修饰键；文件仍是标准 MIDI，其他软件照常打开。
...
src/
  ...
  components/  UI 组件
docs/          映射假设与路线图
desktop/       WPF 桌面版自动演奏器（见 docs/DESKTOP.md）
scripts/       模型拷贝与桌面端黄金样本生成
```
