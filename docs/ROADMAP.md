...
宏版的执行端**不自己实现**：浏览器无法向游戏发送系统级按键，本项目只负责编排并导出，注入交给已有工具（如 [AutoMidiPlayer](https://github.com/Jed556/AutoMidiPlayer)、AutoHotkey 系脚本）。导出格式候选：映射后的 `.mid`、AHK 脚本、带时间戳的键序 JSON。
