...
载入谱面后，点击主窗口底部的「打开悬浮窗」。悬浮窗独立于演奏模式，可与自动演奏同时开启；底部用「手动演奏 / 自动演奏」两选开关切换模式，首次启动默认手动，之后记住选择。手动模式的开始按钮只驱动音符时间轴，不发送按键，并确保悬浮窗已打开。开始 / 停止沿用设置里的全局热键（默认 F5 / F6），手动模式至少预留 3 秒倒计时。停止后再次开始会从头播放。
...
游戏需使用无边框或窗口模式；普通桌面置顶窗不保证覆盖独占全屏。自动模式按实际编排后的按下 / 松开时间显示（包括时序档位产生的延迟），手动模式按原谱时间显示，不做按键判分。
